...
import numpy as np
players = pd.read_csv(r'test.csv')
players.head()
# 中文和负号的正常显示
...
sns.lmplot(x = 'x1',y = 'x3',data = players,
           fit_reg = False, scatter_kws = {'alpha':0.8,'color':'steelblue'})
plt.show()

sns.lmplot(x = 'x2',y = 'x3',data = players,
           fit_reg = False, scatter_kws = {'alpha':0.8,'color':'steelblue'})
plt.show()
...
# 数据标准化处理
X = preprocessing.minmax_scale(players[['x1','x2','x3']])
# 将数组转换为数据框
...
for i in players.cluster.unique():
    centers.append(players.loc[players.cluster == i,['x1','x2','x3']].mean())
# 将列表转换为数组，便于后面的索引取数
centers = np.array(centers)

# 绘制散点图
sns.lmplot(x = 'x1', y = 'x2', hue = 'cluster', data = players, markers = ['^','s','o'],
